...
void CmdExtractExt::DoExtract(CommandData *Cmd, FileHandle FH)
{
  ...
  while (Cmd->GetArcName(ArcName,ArcNameW,ASIZE(ArcName)))
  {
    while (true)
    {
      ...
      EXTRACT_ARC_CODE Code=ExtractArchive(Cmd, FH);
...
void CmdExtractExt::ExtractArchiveInit(CommandData *Cmd,Archive &Arc)
{
...
EXTRACT_ARC_CODE CmdExtractExt::ExtractArchive(CommandData *Cmd, FileHandle FH)
{
  Archive Arc(Cmd);
  ...
  {
     ((FileExt*)&Arc)->SetHandle(FH);
     ((FileExt*)&Arc)->SkipHandle();
  }
...
bool CmdExtractExt::ExtractCurrentFile(CommandData *Cmd,Archive &Arc,size_t HeaderSize,bool &Repeat)
{
...
void CmdExtractExt::UnstoreFile(ComprDataIO &DataIO,int64 DestUnpSize)
{
